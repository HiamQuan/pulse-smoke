...
(function () {
    var b = 0;
    var c = ["o", "ms", "webkit", "moz"];
    for (let i = 0; i < c.length && !window.requestAnimationFrame; ++i) {
        window.requestAnimationFrame = window[c[i] + "RequestAnimationFrame"];
        window.cancelAnimationFrame =
            window[c[i] + "CancelAnimationFrame"] || window[c[i] + "CancelRequestAnimationFrame"];
    }
    if (!window.requestAnimationFrame) {
        window.requestAnimationFrame = function (h, e) {
            var time = new Date().getTime();
            var f = Math.max(0, 16 - (time - b));
    ...
    if (!window.cancelAnimationFrame) {
        window.cancelAnimationFrame = function (evt) {
            clearTimeout(evt);
        };
    }
})();
...
(function (canvas) {
    ...
    function render() {
        ...
        for (var i = 0; i < amount; i++) {
            var pos = moveInnerHeart(Math.PI - 2 * Math.PI * Math.random());
